...
@objc public class FilePicker: NSObject {
    ...
    public func convertMovToMp4(_ sourceUrl: URL) throws -> URL? {
        do {
            let asset = AVAsset(url: sourceUrl)
            let composition = AVMutableComposition()
            guard let videoAssetTrack = asset.tracks(withMediaType: .video).first else {
                throw NSError(domain: "FilePicker", code: -1, userInfo: [NSLocalizedDescriptionKey: "Video track not found"])
            }
            ...
            let directory = try self.createUniqueTemporaryDirectory()
            let filenameWithoutExtension = sourceUrl.deletingPathExtension().lastPathComponent
            let targetUrl = directory.appendingPathComponent("\(filenameWithoutExtension).mp4")
            do {
                try deleteFile(targetUrl)
            }
            ...
            if FileManager.default.fileExists(atPath: targetUrl.path) {
                ...
                throw NSError(domain: "FilePicker", code: -1, userInfo: [NSLocalizedDescriptionKey: "Failed to convert mov to mp4"])
            }
        }
        catch let error as NSError {
            CAPLog.print("convertMovToMp4 failed.", error.localizedDescription)
            return nil
        }
    }
